            ...
            <div class="inline-block min-w-full py-2 align-middle sm:px-6 lg:px-8">
                <div class="overflow-hidden shadow ring-1 ring-black dark:ring-gray-800 ring-opacity-5 sm:rounded-lg">
                    <table>
                        <thead>
                            <tr>
                                <th scope="col" class=" sm:pl-6"> Title
                                    <x-admin.sort-by :sortBy="$sortBy" :sortDirection="$sortDirection" field_name="title" />
                                </th>
                                <th scope="col"> Slug
                                    <x-admin.sort-by :sortBy="$sortBy" :sortDirection="$sortDirection" field_name="slug" />
                                </th>
                                <th scope="col"> Image
                                </th>
                                <th scope="col"> Status
                        ...
                        <tbody>
                            @forelse ($posts as $post)
                                <tr>
                                    <td class="sm:pl-6"> {{ $post->title }} </td>
                                    <td class="sm:pl-6"> {{ $post->slug }} </td>
                                    <td class="sm:pl-6"> 
                                        @if ($post->image)
                                            <img class=" h-20" src="{{ asset('/storage/'.$post->image) }}">
                                        @endif
                                    </td>
                                    <td>
                                        <x-admin.span-status :status="$post->is_active">
                                            {{ $post->status }}
                                        </x-admin.span-status>
                                    </td>
                            ...
                            @empty
                                <tr>
                                    <td colspan="5" class="sm:pl-6"> No posts Found </td>
                                </tr>
                            @endforelse
                            @if ($posts->lastPage() >1)
                                <tr>
